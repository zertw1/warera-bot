...
def load_config():
    return {
        ...
        "discord_bot_token": "6774d4046fbc5fe5664a12ab7001732e64cfadd108a40a971d45316f32a49656"
    }
...
def create_discord_bot(pool):
    ...
    @bot.command()
    async def start(ctx):
        chat_id = str(ctx.author.id)
        await db.update_user(pool, chat_id, 'discord', active=1)
        user = await db.get_user(pool, chat_id, 'discord')
        await ctx.send(f"👋 Welcome! Your settings: Threshold={user['threshold']}, Min Pool={user['min_pool']}")
    ...
    async def status(ctx):
        ...
        user = await db.get_user(pool, chat_id, 'discord')
        if user:
            await ctx.send(f"📊 Settings: Threshold={user['threshold']}, Min Pool={user['min_pool']}, Active={user['active']}")
        else:
            await ctx.send("You are not registered. Use /start to begin.")

    @bot.command()
    async def stop(ctx):
        chat_id = str(ctx.author.id)
        await db.update_user(pool, chat_id, 'discord', active=0)
        await ctx.send("🚫 Notifications stopped. /start to resume.")
...
async def start_background_tasks(app):
    ...
    if discord_token:
        discord_bot_client = create_discord_bot(pool)
        app['discord_bot_client'] = discord_bot_client
        app['discord_task'] = asyncio.create_task(discord_bot_client.start(discord_token))
    else:
